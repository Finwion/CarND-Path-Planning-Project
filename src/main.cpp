...
int main() {
  ...
  while (getline(in_map_, line)) {
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
	  ...
	  //take the last of the previous path's s value and use as reference
	  if(prev_size > 0) 
	    {
	      car_s = end_path_s;
	    }

	  // Go through all cars in the sensor fusion data
	  for(int i = 0; i < sensor_fusion.size(); i++) {
	    float d = sensor_fusion[i][6]; // d will help indicate which lane the cars are in
	  ...
	  if(too_close){
	    //go left if there is no car there
	    //lane_change_cnt to prevent changing lanes twice at one time
	    //cout << car_on_left << " " << car_on_right << endl;
	    ...
	    //slower than speed limit - we can speed up
	  ...
	  if(prev_size < 2) {
	    //previous list of points is nearly empty, use car's coordinates to start
	    ...
	    //previous list of point isn't empty, just use the last 2 points
	  ...
	  //For Frenet get the projected position 30, 60, 90 meters ahead
	  ...
	  //use spline to fit curves through projected positions
	  tk::spline s;
	  ...
	  double x_add_on = 0;
	  //get equally spaced spline points so that we meet the target velocity
	  for(int i = 1; i < 50 - previous_path_x.size(); i++){
	    ...
	    //transform to global x,y reference
